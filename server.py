...
def index():
	...
	return redirect('homepage.html', var=thing)
...
def index():
	...
	#return redirect('homepage.html', var=thing)  #<——redirect to homepage if no login
...
def index():
	...
	thing=request.forms.get('key')
	that=request.args.get('key')
	return redirect('user_profile.html', var=thing) #<——redirect to homepage!
...
#Art Results:
@app.route('/display-art', methods = ['GET', 'POST'])
def index():
